...
| done | Current HEAD production deployment | Owner authorized deploying `develop-saas` latest HEAD `f634bc0` to `smart-return-system-saas`; deployed to Vercel deployment `dpl_2YWna1ojcAQQ5YbQ2SByKxd5oJot` (Ready), with public/protected-route and SEO metadata smoke passing |
...
| blocked | SaaS public rollout external setup | Sentry setup, owner-authorized migrations `035` and `037`, tenant isolation P0/P1/P2 reviewed-path hardening/gating, and latest owner-authorized production deployment are complete; production is Ready at `f634bc0` / `dpl_2YWna1ojcAQQ5YbQ2SByKxd5oJot`, but latest pushed source is now `8ae2aa7` and read-only production smoke shows `/pricing` still exposes old `1,490` / `2,990` pricing markers. The 499/699 pricing and multi-channel honesty copy are in Git but require a separately owner-authorized production deploy before prospects see them. Owner confirmed `smart-return.tw` has not been purchased and chose to continue Closed Manual Beta on the Vercel production URL, so custom domain work is deferred and should not be retried until the owner buys/registers a domain and reauthorizes DNS/Vercel verification; Resend is the recommended first email provider but requires owner account/domain/API key/sender/scope decisions and a future provider-adapter task before enabling delivery; next queue is deploy latest `develop-saas` if public site should be current, then `038`, read-only production `/internal` verification, disposable-org QA, public signup posture, Resend/email delivery authorization, Stage 2 Billing/ECPay, then separately authorized `033`/`034`/`036` |
