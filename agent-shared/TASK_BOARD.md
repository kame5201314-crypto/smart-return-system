...
| blocked | SaaS public rollout external setup | Latest read-only refresh confirmed `develop-saas` is synced at `4a1d7f8` and includes Claude UI improvements through `615ce7c`, but production still remains on the earlier Closed Manual Beta deployment `dpl_8Huiefp9Y3A3W3Wxpsvsx4WFDajS`; Vercel env lacks `SENTRY_DSN` / `NEXT_PUBLIC_SENTRY_DSN`, no custom domain is configured, ECPay credentials are absent, email delivery remains dry-run, draft migrations `033`-`036` are not applied, and any latest-HEAD deploy/platform setting change still needs explicit owner approval |
...
- External/owner-blocked items remain Sentry DSN activation, Billing/ECPay plus `ENABLE_BILLING`, beta/custom domain/DNS, email provider delivery, applying draft migrations `033`-`036`, and any production deploy or platform setting change. Latest read-only Vercel check confirmed `develop-saas` is synced at `4a1d7f8`, production remains on Ready deployment `dpl_8Huiefp9Y3A3W3Wxpsvsx4WFDajS` until owner authorizes a latest-HEAD deploy, and no Sentry DSN, custom domains, or ECPay env values are configured.
