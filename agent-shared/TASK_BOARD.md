...
| blocked | SaaS public rollout external setup | 2026-06-06 Sentry setup removed the Sentry blocker and production is Ready at `360c56f` / `dpl_FjkpCWZwYPSv7RY2sBJEhpFPPMab`; next queue is custom/beta domain, migration `035`, email provider, Stage 2 Billing/ECPay, then separately authorized `033`/`034`/`036`; any deploy/platform/env/domain/provider/migration change still needs explicit owner approval plus real values |
...
- No unblocked backend/API/migration task is currently open after the explicit platform admin identity separation, launch security hardening, platform admin login throttling, mutation same-origin hardening, public signup rate limiting, non-UI lint cleanup, onboarding, platform dashboard, tenant preview, audit, external blocker refresh, and latest UI handoffs. Public multi-tenant expansion should not proceed until the tenant-isolation P1/P2 gaps documented in `docs/SAAS_TENANT_ISOLATION_AUDIT.md` are explicitly scheduled and hardened or gated.
...
- External/owner-blocked items should be handled in this order: beta/custom domain/DNS, migration `035` if onboarding completion writes should persist, email provider delivery, Stage 2 Billing/ECPay plus `ENABLE_BILLING`, then separately authorized draft migrations `033`/`034`/`036`. Sentry DSN activation is complete and production is currently Ready on `dpl_FjkpCWZwYPSv7RY2sBJEhpFPPMab`; no additional safe Codex action is available until the owner provides real values or explicit per-action authorization.
