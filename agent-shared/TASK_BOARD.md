...
| blocked | SaaS public rollout external setup | Sentry setup, owner-authorized migrations `035`, `037`, and `038`, tenant isolation P0/P1/P2 reviewed-path hardening/gating, latest owner-authorized production deployment, production admin verification, and disposable QA team-management verification are complete; production is Ready at `3fadd75` / `dpl_2ELVrGvkGzEF47juNTZA9yu5UV76`, and smoke confirms `/pricing` now exposes the 499/699 pricing markers plus the multi-channel honesty copy. Owner confirmed `smart-return.tw` has not been purchased and chose to continue Closed Manual Beta on the Vercel production URL, so custom domain work is deferred and should not be retried until the owner buys/registers a domain and reauthorizes DNS/Vercel verification; Resend is the recommended first email provider but requires owner account/domain/API key/sender/scope decisions and a future provider-adapter task before enabling delivery; remaining owner-blocked queue is public signup posture, invoice/legal finalization before paid customers, Resend/email delivery authorization, Stage 2 Billing/ECPay, optional `PLATFORM_ADMIN_ROLES`, then separately authorized `033`/`034`/`036` |
...
- External/owner-blocked items should be handled in this order: confirm invoice/receipt capability and legal-page finalization before the first paid customer, decide public signup posture, keep email delivery disabled until owner provides Resend credentials and authorizes provider wiring, Stage 2 Billing/ECPay plus `ENABLE_BILLING`, optional `PLATFORM_ADMIN_ROLES` if per-email platform operator roles are needed, then separately authorized draft migrations `033`/`034`/`036`. Sentry DSN activation, migrations `035`, `037`, and `038`, reviewed tenant-isolation hardening/gating, latest owner-authorized production deployment, production admin authenticated verification, disposable QA team-management verification, local go-live gates, manual payment/support/privacy SOPs, and the Closed Beta onboarding runbook are complete; production is currently Ready on `dpl_2ELVrGvkGzEF47juNTZA9yu5UV76` from runtime source `3fadd75`, and production `/pricing` now shows the 499/699 markers plus multi-channel honesty copy. Custom domain work is deferred because the owner has not purchased `smart-return.tw` and chose to keep using the Vercel production URL for Closed Manual Beta. This documentation update may create a newer docs-only commit than the deployed runtime source.
