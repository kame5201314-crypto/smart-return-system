...
| done | Current HEAD production deployment | This commit; owner authorized deploying current `origin/develop-saas` HEAD to `smart-return-system-saas`; deployed `0c9c983` containing `27c5ecb` to Vercel deployment `dpl_EwmXZXdxNAYHZdoBNRHN5kQnW7yu`, with public/protected-route smoke passing |
...
| blocked | SaaS public rollout external setup | 2026-06-06 Sentry setup, owner-authorized migration `035`, tenant isolation P0/P1/P2 reviewed-path hardening/gating, and current-head production deployment are complete; production is Ready at `0c9c983` / `dpl_EwmXZXdxNAYHZdoBNRHN5kQnW7yu`; owner selected and authorized setup for `app.smart-return.tw`, but latest retry still shows DNS NXDOMAIN, Vercel 403 domain access, 0 domains under the current Vercel scope, and no CLI-returned TXT ownership record, so owner/DNS action is required before retry; owner skipped email provider for now; next queue is DNS/ownership verification for `app.smart-return.tw`, public signup posture, Stage 2 Billing/ECPay, then separately authorized `033`/`034`/`036` |
...
- External/owner-blocked items should be handled in this order: complete DNS/ownership verification for selected domain `app.smart-return.tw`, decide public signup posture, keep email provider skipped until credentials are provided, Stage 2 Billing/ECPay plus `ENABLE_BILLING`, then separately authorized draft migrations `033`/`034`/`036`. Sentry DSN activation, migration `035`, current-head production deployment, and one Vercel domain setup attempt are complete; production is currently Ready on `dpl_EwmXZXdxNAYHZdoBNRHN5kQnW7yu` and includes `27c5ecb`. Latest pushed source HEAD is `b2fc132`, but it has not been deployed to production.
