...
| done | AI returns to platform admin QA contract | This commit; added platform-admin usage aggregation tests proving `/internal` usage summaries are sourced from org-scoped `return_requests` and non-cached successful `ai_usage_events`, added `docs/SAAS_AI_RETURNS_PLATFORM_QA_PLAN.md`, and drafted migration `038_saas_org_member_visibility.sql` to unblock owner/admin team-member visibility without recursive RLS. No migration, deploy, env, billing, provider, or domain change was performed |
