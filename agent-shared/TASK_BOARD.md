...
| blocked | SaaS public rollout external setup | 2026-06-06 Sentry setup and owner-authorized migration `035` are complete; latest pushed SaaS commit is `27c5ecb`, while production is still Ready at `360c56f` / `dpl_FjkpCWZwYPSv7RY2sBJEhpFPPMab`; tenant isolation P0/P1/P2 reviewed paths are locally hardened/gated and should be deployed only after explicit owner deploy authorization; owner selected `app.smart-return.tw` but domain/DNS changes still need explicit authorization; owner skipped email provider for now; next queue is latest-head deploy authorization, custom domain authorization, public signup posture, Stage 2 Billing/ECPay, then separately authorized `033`/`034`/`036` |
...
- External/owner-blocked items should be handled in this order: deploy latest pushed `27c5ecb` only after explicit Vercel production deploy authorization, configure selected domain `app.smart-return.tw` only after explicit Vercel/DNS authorization, decide public signup posture, keep email provider skipped until credentials are provided, Stage 2 Billing/ECPay plus `ENABLE_BILLING`, then separately authorized draft migrations `033`/`034`/`036`. Sentry DSN activation and migration `035` are complete, and production is currently Ready on `dpl_FjkpCWZwYPSv7RY2sBJEhpFPPMab`.
