...
| blocked | SaaS public rollout external setup | 2026-06-06 Sentry setup, owner-authorized migration `035`, tenant isolation P0/P1/P2 reviewed-path hardening/gating, and current-head production deployment are complete; production is Ready at `0c9c983` / `dpl_EwmXZXdxNAYHZdoBNRHN5kQnW7yu`; owner selected `app.smart-return.tw` but domain/DNS changes still need explicit authorization; owner skipped email provider for now; next queue is custom domain authorization, public signup posture, Stage 2 Billing/ECPay, then separately authorized `033`/`034`/`036` |
...
- External/owner-blocked items should be handled in this order: configure selected domain `app.smart-return.tw` only after explicit Vercel/DNS authorization, decide public signup posture, keep email provider skipped until credentials are provided, Stage 2 Billing/ECPay plus `ENABLE_BILLING`, then separately authorized draft migrations `033`/`034`/`036`. Sentry DSN activation, migration `035`, and current-head production deployment are complete; production is currently Ready on `dpl_EwmXZXdxNAYHZdoBNRHN5kQnW7yu` and includes `27c5ecb`.
