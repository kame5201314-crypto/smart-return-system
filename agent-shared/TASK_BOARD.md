...
| blocked | SaaS public rollout external setup | 2026-06-06 Sentry setup and owner-authorized migration `035` are complete; production is Ready at `360c56f` / `dpl_FjkpCWZwYPSv7RY2sBJEhpFPPMab`; next queue is custom/beta domain, email provider, Stage 2 Billing/ECPay, then separately authorized `033`/`034`/`036`; any deploy/platform/env/domain/provider/migration change still needs explicit owner approval plus real values |
...
- External/owner-blocked items should be handled in this order: beta/custom domain/DNS, email provider delivery, Stage 2 Billing/ECPay plus `ENABLE_BILLING`, then separately authorized draft migrations `033`/`034`/`036`. Sentry DSN activation and migration `035` are complete, and production is currently Ready on `dpl_FjkpCWZwYPSv7RY2sBJEhpFPPMab`; no additional safe Codex action is available until the owner provides real values or explicit per-action authorization.
