...
- External/owner-blocked items should be handled in this order: decide public signup posture, apply `037` only if the owner wants full `/settings/team` invite revoke/resend QA on the real SaaS DB, keep email delivery disabled until owner provides Resend credentials and authorizes provider wiring, Stage 2 Billing/ECPay plus `ENABLE_BILLING`, then separately authorized draft migrations `033`/`034`/`036`/`037`. Sentry DSN activation, migration `035`, reviewed tenant-isolation hardening/gating, and latest production deployment are complete; production is currently Ready on `dpl_2YWna1ojcAQQ5YbQ2SByKxd5oJot` from runtime source `f634bc0`, and `https://smart-return-system-saas.vercel.app` smoke passes. Custom domain work is deferred because the owner has not purchased `smart-return.tw` and chose to keep using the Vercel production URL for Closed Manual Beta. This documentation update may create a newer docs-only commit than the deployed runtime source.
