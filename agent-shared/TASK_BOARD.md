...
| blocked | SaaS public rollout external setup | Sentry setup, owner-authorized migration `035`, tenant isolation P0/P1/P2 reviewed-path hardening/gating, and latest owner-authorized production deployment are complete; production is Ready at `f634bc0` / `dpl_2YWna1ojcAQQ5YbQ2SByKxd5oJot`; latest smoke confirms `https://smart-return-system-saas.vercel.app` public and protected-route checks pass; Vercel Dashboard lists `app.smart-return.tw` under project `smart-return-system-saas`, but it is `Invalid Configuration`; no TXT ownership challenge is visible, and the required DNS record is `CNAME app -> 64ed959ebaa2a805.vercel-dns-016.com.`; local DNS/RDAP checks still show `smart-return.tw` / `app.smart-return.tw` unresolved, so owner must confirm the root domain is registered/delegated and add the dashboard CNAME at the authoritative DNS provider; Resend is the recommended first email provider but requires owner account/domain/API key/sender/scope decisions and a future provider-adapter task before enabling delivery; next queue is DNS/SSL verification for `app.smart-return.tw`, public signup posture, Resend/email delivery authorization, Stage 2 Billing/ECPay, then separately authorized `033`/`034`/`036` |
...
- External/owner-blocked items should be handled in this order: complete DNS/SSL verification for selected domain `app.smart-return.tw`, decide public signup posture, keep email delivery disabled until owner provides Resend credentials and authorizes provider wiring, Stage 2 Billing/ECPay plus `ENABLE_BILLING`, then separately authorized draft migrations `033`/`034`/`036`. Sentry DSN activation, migration `035`, reviewed tenant-isolation hardening/gating, and latest production deployment are complete; production is currently Ready on `dpl_2YWna1ojcAQQ5YbQ2SByKxd5oJot` from runtime source `f634bc0`, and `https://smart-return-system-saas.vercel.app` smoke passes. The selected custom domain remains blocked because Vercel Dashboard shows `app.smart-return.tw` as `Invalid Configuration` and local DNS/RDAP still does not resolve the root/app domain; owner must confirm domain registration/delegation and set `CNAME app -> 64ed959ebaa2a805.vercel-dns-016.com.` at the DNS provider. This documentation update may create a newer docs-only commit than the deployed runtime source.
