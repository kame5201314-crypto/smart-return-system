...
| blocked | SaaS public rollout external setup | Sentry setup, owner-authorized migration `035`, tenant isolation P0/P1/P2 reviewed-path hardening/gating, and latest owner-authorized production deployment are complete; production is Ready at `f634bc0` / `dpl_2YWna1ojcAQQ5YbQ2SByKxd5oJot`; latest smoke confirms `https://smart-return-system-saas.vercel.app` public and protected-route checks pass; Vercel still lists `app.smart-return.tw` as an alias, but DNS for `app.smart-return.tw` / `smart-return.tw` is NXDOMAIN, HTTPS cannot resolve the host, Vercel domain inspect returns 403, and `vercel domains ls` shows 0 domains in the current scope; owner/DNS verification is still required before using the custom domain; owner skipped email provider for now and no email/ECPay credential names are visible in Vercel Production env; next queue is DNS/SSL verification for `app.smart-return.tw`, public signup posture, Stage 2 Billing/ECPay, then separately authorized `033`/`034`/`036` |
