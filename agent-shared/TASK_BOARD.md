...
- External/owner-blocked items should be handled in this order: complete DNS/SSL verification for selected domain `app.smart-return.tw`, decide public signup posture, keep email delivery disabled until owner provides Resend credentials and authorizes provider wiring, Stage 2 Billing/ECPay plus `ENABLE_BILLING`, then separately authorized draft migrations `033`/`034`/`036`. Sentry DSN activation, migration `035`, reviewed tenant-isolation hardening/gating, and latest production deployment are complete; production is currently Ready on `dpl_2YWna1ojcAQQ5YbQ2SByKxd5oJot` from runtime source `f634bc0`, and `https://smart-return-system-saas.vercel.app` smoke passes. The selected custom domain remains blocked because DNS is NXDOMAIN and Vercel domain ownership is not visible from the current scope. This documentation update may create a newer docs-only commit than the deployed runtime source.
