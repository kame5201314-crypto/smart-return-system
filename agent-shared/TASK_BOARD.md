...
| blocked | SaaS public rollout external setup | Sentry setup, owner-authorized migration `035`, tenant isolation P0/P1/P2 reviewed-path hardening/gating, and latest owner-authorized production deployment are complete; production is Ready at `796a02a` / `dpl_28RhEVo2Nespq7xjTEQvmELag34r`; Vercel auto-listed `app.smart-return.tw` as an alias during deploy, but DNS still does not resolve and SSL was asynchronous, so owner/DNS verification is still required before using the custom domain; owner skipped email provider for now; next queue is DNS/SSL verification for `app.smart-return.tw`, public signup posture, Stage 2 Billing/ECPay, then separately authorized `033`/`034`/`036` |
...
- External/owner-blocked items should be handled in this order: complete DNS/SSL verification for selected domain `app.smart-return.tw`, decide public signup posture, keep email provider skipped until credentials are provided, Stage 2 Billing/ECPay plus `ENABLE_BILLING`, then separately authorized draft migrations `033`/`034`/`036`. Sentry DSN activation, migration `035`, reviewed tenant-isolation hardening/gating, and latest production deployment are complete; production is currently Ready on `dpl_28RhEVo2Nespq7xjTEQvmELag34r` from runtime source `796a02a`. This documentation update may create a newer docs-only commit than the deployed runtime source.
