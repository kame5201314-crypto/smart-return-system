...
| blocked | SaaS public rollout external setup | 2026-06-06 Sentry setup and owner-authorized migration `035` are complete; owner selected `app.smart-return.tw` but domain/DNS changes still need explicit authorization; owner skipped email provider for now; production is Ready at `360c56f` / `dpl_FjkpCWZwYPSv7RY2sBJEhpFPPMab`; tenant isolation P0/P1/P2 reviewed paths are locally hardened/gated; next queue is custom domain authorization, public signup posture, Stage 2 Billing/ECPay, then separately authorized `033`/`034`/`036` |
