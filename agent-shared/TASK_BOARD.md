...
| done | Current HEAD production deployment | This commit; owner authorized deploying `develop-saas` latest HEAD `796a02a` to `smart-return-system-saas`; deployed to Vercel deployment `dpl_28RhEVo2Nespq7xjTEQvmELag34r` (Ready), with public/protected-route smoke passing |
...
| blocked | SaaS public rollout external setup | Sentry setup, owner-authorized migration `035`, tenant isolation P0/P1/P2 reviewed-path hardening/gating, and latest owner-authorized production deployment are complete; production is Ready at `796a02a` / `dpl_28RhEVo2Nespq7xjTEQvmELag34r`; latest post-rollout recheck confirms `https://smart-return-system-saas.vercel.app` smoke passes, while Vercel still lists `app.smart-return.tw` as an alias but DNS for `app.smart-return.tw` / `smart-return.tw` is NXDOMAIN, HTTPS cannot resolve the host, domain inspect returns 403, and `vercel domains ls` shows 0 domains in the current scope; owner/DNS verification is still required before using the custom domain; owner skipped email provider for now; next queue is DNS/SSL verification for `app.smart-return.tw`, public signup posture, Stage 2 Billing/ECPay, then separately authorized `033`/`034`/`036` |
