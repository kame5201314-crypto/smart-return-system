...
| done | Public signup rate limit hardening | This commit; `POST /api/saas/signup` now has best-effort per-runtime request throttling by scope, client IP, and user agent, with unit tests and SaaS doctor coverage. Public signup remains disabled by default |
