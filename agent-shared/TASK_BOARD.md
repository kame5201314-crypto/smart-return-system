...
| blocked | SaaS public rollout external setup | 2026-06-06 Sentry setup removed the Sentry blocker and production is Ready at `360c56f` / `dpl_FjkpCWZwYPSv7RY2sBJEhpFPPMab`; no custom/beta domain is visible, ECPay/provider credentials are absent, email delivery remains dry-run, draft migrations `033`-`036` are not applied, and any further deploy/platform/env/domain/provider/migration change still needs explicit owner approval plus real values |
...
- External/owner-blocked items remain Billing/ECPay plus `ENABLE_BILLING`, beta/custom domain/DNS, email provider delivery, applying draft migrations `033`-`036`, and any further production deploy or platform setting change. Sentry DSN activation is complete and production is currently Ready on `dpl_FjkpCWZwYPSv7RY2sBJEhpFPPMab`; no additional safe Codex action is available until the owner provides real values or explicit per-action authorization.
