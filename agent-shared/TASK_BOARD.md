...
- External/owner-blocked items should be handled in this order: run authenticated platform-admin login QA with existing admin credentials or set `PLATFORM_ADMIN_ROLES` if per-email operator roles are needed, run merchant-to-platform QA with a disposable org, confirm invoice/receipt capability and legal-page finalization before the first paid customer, decide public signup posture, keep email delivery disabled until owner provides Resend credentials and authorizes provider wiring, Stage 2 Billing/ECPay plus `ENABLE_BILLING`, then separately authorized draft migrations `033`/`034`/`036`. Sentry DSN activation, migrations `035`, `037`, and `038`, reviewed tenant-isolation hardening/gating, latest owner-authorized production deployment, production admin read-only verification, local go-live gates, manual payment/support/privacy SOPs, and the Closed Beta onboarding runbook are complete; production is currently Ready on `dpl_2ELVrGvkGzEF47juNTZA9yu5UV76` from runtime source `3fadd75`, and production `/pricing` now shows the 499/699 markers plus multi-channel honesty copy. Custom domain work is deferred because the owner has not purchased `smart-return.tw` and chose to keep using the Vercel production URL for Closed Manual Beta. This documentation update may create a newer docs-only commit than the deployed runtime source.
