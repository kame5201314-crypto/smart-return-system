...
CREATE TABLE IF NOT EXISTS public.return_requests (
    ...
    status VARCHAR(30) DEFAULT 'pending_review', -- pending_review, approved, rejected, processing, shipped_back, received, inspecting, completed, cancelled
    reason_category VARCHAR(50), -- quality_issue, wrong_item, damaged_in_transit, not_as_described, change_of_mind, installation_issue, defective, size_not_fit, other
    reason_detail TEXT,
    review_notes TEXT,
    refund_amount DECIMAL(12,2),
    refund_type VARCHAR(30), -- full_refund, partial_refund, store_credit
...
CREATE TABLE IF NOT EXISTS public.return_images (
    ...
    storage_path TEXT,
    image_type VARCHAR(50), -- product_damage, shipping_label, outer_box, inspection, other
...
CREATE TABLE IF NOT EXISTS public.activity_logs (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    entity_type VARCHAR(50) NOT NULL, -- return_request, order, customer
    entity_id UUID NOT NULL,
    action VARCHAR(50) NOT NULL, -- created, updated, status_changed, etc.
    actor_type VARCHAR(50), -- customer, staff, system
    actor_id UUID,
    description TEXT,
    old_value JSONB,
    new_value JSONB,
...
CREATE TABLE IF NOT EXISTS public.inspection_records (
    ...
    condition_grade VARCHAR(10), -- A, B, C, D, F
...
-- ============================================
-- 8. AI 分析報告表 (AI 分析頁面需要)
-- ============================================
CREATE TABLE IF NOT EXISTS public.ai_analysis_reports (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    report_period VARCHAR(10) NOT NULL, -- e.g., '2024-01'
...
-- ============================================
-- 10. RLS Policies - 允許 service_role 完整存取
-- ============================================
-- customers
